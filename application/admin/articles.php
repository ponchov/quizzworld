...
class Articles extends CI_Controller {       
	...
	{	
		$post_type="articles";
		$this->session->set_userdata('article_search','');
		$this->session->set_userdata('search_status','');
		$total_rows = $this->test_model->get_total_tests($lang_code,$post_type);
		
		$limit=30;
		$this->load->library('pagination');
	
		$config['base_url'] = site_url().'admin/articles/index/'.$lang_code;
        ...
        $db_data['test_list']=$this->test_model->get_tests($lang_code,$page,$limit,$post_type);	
		$db_data['lang']=$lang_code;
		$db_data['language_list']=$this->test_model->get_languages();	

		$data['content'] = $this->load->view('admin/articles/list',$db_data,true);

		//----- this is for breadcrumbs

		$breadcrumbs['breadcrumbs'][]=array('text'=>'Dashboard','href'=>site_url().'admin');

		$breadcrumbs['breadcrumbs'][]=array('text'=>'Article List','href'=>'');
	...
	function search_tests($lang_code='en',$page=0)
	{
		...
		{
			$this->session->set_userdata('article_search',$this->input->post('search'));
		}
		...
		$total_rows = $this->test_model->get_total_search_tests($lang_code,'articles');
		
		$limit=30;
		$this->load->library('pagination');
	
		$config['base_url'] = site_url().'admin/articles/search_tests/'.$lang_code;
		$config['uri_segment'] = 5;
		$config['num_links'] = 10;
		$config['prev_link'] = 'Pre';
		$config['next_link'] = 'Next';
        ...
        $db_data['test_list']=$this->test_model->get_search_tests($lang_code,$page,$limit,'articles');	
		$db_data['lang']=$lang_code;
		$db_data['language_list']=$this->test_model->get_languages();	
	...
	function add($lan='en')
	{
		if($this->input->post('add_test'))
		{
			...
			$meta_info[]=array(
						'test_id'=>$id,
						'meta_key'=>'article_snippet',
						'meta_value'=>$this->input->post('article_snippet')
						);
						
			/*$meta_info[]=array(
						'test_id'=>$id,
						'meta_key'=>'article_snippet_2',
						'meta_value'=>$this->input->post('article_snippet_2')
						);
			$meta_info[]=array(
						'test_id'=>$id,
						'meta_key'=>'article_snippet_3',
						'meta_value'=>$this->input->post('article_snippet_3')
						);*/
			...
			$this->session->set_flashdata('success_message','Article  has been saved successfully');
			
			redirect('admin/articles/index/'.$lan);

		}

		else
		{
			$db_data['category_list']=$this->test_model->get_categories($lan,'articles');
			$db_data['lan']=$lan;
			$data['content']=$this->load->view('admin/articles/add_form',$db_data,true);

			//----- this is for breadcrumbs

			$breadcrumbs['breadcrumbs'][]=array('text'=>'Articles','href'=>site_url().'admin/articles');

			$breadcrumbs['breadcrumbs'][]=array('text'=>"Add new",'href'=>'');

			$data['breadcrumbs']=$this->load->view('admin/layout/breadcrumbs',$breadcrumbs,true);	

			$data['page_header']="Article Add ";
		...
		if($this->input->post('edit_test'))
		{
			...
			if($this->form_validation->run()) 
			{
				...
				//{
					...
					//----------upload thumbs image
					$this->image_lib->clear();
					$config2['upload_path'] = 'assets/img/thumbs/';
					...
					$this->test_model->edit_testMeta($meta_info,$id,'article_snippet');
					/*$meta_info=array(
								'meta_value'=>$this->input->post('article_snippet_2')
								);
					$this->test_model->edit_testMeta($meta_info,$id,'article_snippet_2');
					$meta_info=array(
								'meta_value'=>$this->input->post('article_snippet_3')
								);
					$this->test_model->edit_testMeta($meta_info,$id,'article_snippet_3');*/
				...
				if($referance && $referance == 'details') redirect('admin/articles/test_details/'.$id."/".$lan);
				else redirect('admin/articles/index/'.$lan);
			}
			else
			{ 
				...
				$data['lan']=$lan;
				$data['content']=$this->load->view('admin/articles/edit_form',$data,true);
				//----- this is for breadcrumbs

				$breadcrumbs['breadcrumbs'][]=array('text'=>'Articles','href'=>site_url().'admin/articles');
				$breadcrumbs['breadcrumbs'][]=array('text'=>$data['test_info']->title,'href'=>'');
				$breadcrumbs['breadcrumbs'][]=array('text'=>"Preview",'href'=>site_url()."admin/articles/test_details/".$data['test_info']->testid."/".$lan);
				$data['breadcrumbs']=$this->load->view('admin/layout/breadcrumbs',$breadcrumbs,true);	
	...
	{ 
		...
		$this->session->set_flashdata('success_message','Articles has been Deleted');
	...
	{	
		...
		$total_rows = $this->test_model->get_total_tests($lang_code,'articles');
		$limit=30;
		$this->load->library('pagination');
        ...
        $db_data['test_list']=$this->test_model->get_tests($lang_code,$page,$limit,'articles');	
		$db_data['lang']=$lang_code;
		$db_data['language_list']=$this->test_model->get_languages();
		$db_data['page_number']=$page;
		echo $data['content'] = $this->load->view('admin/articles/test_list_ajax',$db_data,true);
	...
	function ads_edit($test_id)
	{
		...
		if($this->input->post('edit_config'))
		{
			...
			redirect('admin/articles/ads_edit/'.$test_id);
		}
		else
		{
	...
	{	
		...
		$data['content'] = $this->load->view('admin/articles/test_details',$db_data,true);
		//----- this is for breadcrumbs
		$breadcrumbs['breadcrumbs'][]=array('text'=>'Dashboard','href'=>site_url().'admin');
	...
	{		
		...
		redirect('admin/articles/test_details/'.$test_id."/".$this->input->post('lang_code'));	
	}
	...
	function test_content($testid,$lang_code)
	{
		...
		if($this->input->post('test_content'))
		{
			...
			if($this->form_validation->run()) 
			{
				...
				$test_content_info['post_type']="articles";
				...
				//----------upload thumbs image
					...
					if($this->upload->do_upload('image_thumb'))
					{
						...
						if($this->input->post('pre_img_path2') != "assets/img/thumbs/".$this->input->post('eng_test_image_thumb'))
						{
							 @unlink($this->input->post('pre_img_path2'));
						}
				...
				$this->session->set_flashdata('success_message',' Article Content has been saved');
			    redirect('admin/articles/index/'.$lang_code);
			...
			else
			{
				...
				$data['testid']=$testid;
				$data['lang_code']=$lang_code;
				$data['category_list']=$this->test_model->get_categories($lang_code,'articles');
				$data['referance']='';
				$data['content']=$this->load->view('admin/articles/test_content_form',$data,true);
				//----- this is for breadcrumbs
				$breadcrumbs['breadcrumbs'][]=array('text'=>'Articles','href'=>site_url().'admin/articles');			
				$breadcrumbs['breadcrumbs'][]=array('text'=>'Article Content','href'=>'');
				$data['breadcrumbs']=$this->load->view('admin/layout/breadcrumbs',$breadcrumbs,true);	
				$data['page_header']="Article Content Form";
				//------- end breadcrumbs
				$data['active_menu']='';
				$this->load->view('admin/layout/default',$data);
			}
	...
	function edit_comments()
	{
		if($this->input->post('edit_comments'))
		{
			...
			redirect('admin/articles/test_details/'.$test_id."/".$lang_code);
		}
		else
		{
