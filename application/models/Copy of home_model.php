...
class Home_model extends CI_Model {
	...
	function get_previous_test_url($test_id,$activated_date='')
	{
	...
	function get_promoted_test($test_id)
	{
		$this->db->select('*');
		$this->db->where('post_type',"games");
		$this->db->where('is_real_test',3);
		...
		{
			$res=$this->get_previous_test($test_id,3);
			if($res) return $res;
			else
			{
				$res=$this->get_next_test($test_id,3);
			}
		}
	}

	function get_next_test_url($test_id,$activated_date='')
