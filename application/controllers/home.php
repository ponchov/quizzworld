...
class Home extends main_site {
	...
	{	
		...
		$db_data['page']=$page;
		$db_data['limit']=$limit;
		$db_data['config_info']=$this->home_model->get_config('config',$this->session->userdata('lang_code'));
		$db_data['ads_info']=$this->home_model->get_ads_unique('personal');
		...
		$db_data['game_type']='all';
		//$db_data['featured_posts']=$this->home_model->get_featured_post();
		//$db_data['top_banner']=$this->home_model->get_topbanners('4');
		//$db_data['random_games']=$this->home_model->get_randon_lists('games','4','','');
		$db_data['widgets']=$this->home_model->get_widget('homesidebar',$this->session->userdata('lang_code'));
	...
	{	
		//echo $this->template; exit;
        $total_rows = $this->home_model->get_total_games('games','','quiz');	
		if($total_rows == 0)
		{
			redirect($this->config->item('root_url'));
		} 
		...
		$config['uri_segment'] = 2;
		$config['num_links'] = 7;
        $config['base_url'] = site_url()."personality-quizz.html";
        $config['total_rows'] = $total_rows;
        $config['per_page'] = $limit;
        $this->pagination->initialize($config);
		$db_data['pagination']=$this->pagination->create_links();
		...
		$db_data['page']=$page;
		$db_data['limit']=$limit;
		$db_data['game_type']='quiz';
		$db_data['ref_page']="games";
		
		$db_data['config_info']=$this->home_model->get_config('config',$this->session->userdata('lang_code'));
		$game_list=$this->home_model->get_games($start,$limit,'games','',"quiz");
		...
		$db_data['widgets']=$this->home_model->get_widget('homesidebar',$this->session->userdata('lang_code'));
		$data['config_info'] = $db_data['config_info'];
		$db_data['quiz_type']=$this->lang->line('personality_quizz');
		$data['content'] = $this->load->view($this->template.'/home/home',$db_data,true);
		$data['page_title'] = $db_data['config_info']->site_title;
		$data['page_description'] = $db_data['config_info']->page_description;
	...
	function trivia($page=0)
	{	
		//echo $this->template; exit;
        $total_rows = $this->home_model->get_total_trivia();	
		if($total_rows == 0)
		{
			redirect($this->config->item('root_url'));
		} 
		...
		$config['uri_segment'] = 2;
		$config['num_links'] = 7;
        $config['base_url'] = site_url()."trivia.html";
        $config['total_rows'] = $total_rows;
        $config['per_page'] = $limit;
        $this->pagination->initialize($config);
		$db_data['pagination']=$this->pagination->create_links();
		...
		$db_data['limit']=$limit;
		$db_data['config_info']=$this->home_model->get_config('config',$this->session->userdata('lang_code'));
		$game_list=$this->home_model->get_trivis($start,$limit);
		
		$db_data['game_type']='trivia';
		$db_data['ref_page']="trivia";
		...
		$db_data['widgets']=$this->home_model->get_widget('homesidebar',$this->session->userdata('lang_code'));
		$data['config_info'] = $db_data['config_info'];
		$db_data['quiz_type']=$this->lang->line('trivia');
		$data['content'] = $this->load->view($this->template.'/home/home',$db_data,true);
		$data['page_title'] = $db_data['config_info']->site_title;
		$data['page_description'] = $db_data['config_info']->page_description;
		$data['meta_url'] = $db_data['config_info']->facebook_url;
	...
	function apps($page=0)
	{	
		//echo $this->template; exit;
        $total_rows = $this->home_model->get_total_apps();	
		if($total_rows == 0)
		{
			redirect($this->config->item('root_url'));
		} 
		...
		$config['uri_segment'] = 2;
		$config['num_links'] = 7;
        $config['base_url'] = site_url()."apps.html";
        $config['total_rows'] = $total_rows;
        $config['per_page'] = $limit;
        $this->pagination->initialize($config);
		$db_data['pagination']=$this->pagination->create_links();
		...
		$db_data['limit']=$limit;
		$db_data['config_info']=$this->home_model->get_config('config',$this->session->userdata('lang_code'));
		$game_list=$this->home_model->get_apps($start,$limit);

		$db_data['game_type']='apps';
		$db_data['ref_page']="apps";
		...
		$db_data['widgets']=$this->home_model->get_widget('homesidebar',$this->session->userdata('lang_code'));
		$data['config_info'] = $db_data['config_info'];
		$db_data['quiz_type']=$this->lang->line('apps');
		$data['content'] = $this->load->view($this->template.'/home/home',$db_data,true);
		$data['page_title'] = $db_data['config_info']->site_title;
		$data['page_description'] = $db_data['config_info']->page_description;
		$data['meta_url'] = $db_data['config_info']->facebook_url;
	...
	function name($page=0)
	{	
		//echo $this->template; exit;
        $total_rows = $this->home_model->get_total_names();	
		if($total_rows == 0)
		{
			redirect($this->config->item('root_url'));
		} 
		...
		$config['uri_segment'] = 2;
		$config['num_links'] = 7;
        $config['base_url'] = site_url()."name.html";
        $config['total_rows'] = $total_rows;
        $config['per_page'] = $limit;
        $this->pagination->initialize($config);
		$db_data['pagination']=$this->pagination->create_links();
		...
		$db_data['limit']=$limit;
		$db_data['config_info']=$this->home_model->get_config('config',$this->session->userdata('lang_code'));
		$game_list=$this->home_model->get_names($start,$limit);

		$db_data['game_type']='name';
		$db_data['ref_page']="name";
		...
		$db_data['widgets']=$this->home_model->get_widget('homesidebar',$this->session->userdata('lang_code'));
		$data['config_info'] = $db_data['config_info'];
		$db_data['quiz_type']=$this->lang->line('name_apps');
		$data['content'] = $this->load->view($this->template.'/home/home',$db_data,true);
		$data['page_title'] = $db_data['config_info']->site_title;
		$data['page_description'] = $db_data['config_info']->page_description;
		$data['meta_url'] = $db_data['config_info']->facebook_url;
	...
	{	
		...
		$db_data['game_type']='lists';
		$db_data['ref_page']="lists";
		...
		$db_data['random_games']=$this->home_model->get_randon_lists('games','4','','');
		$data['config_info'] = $db_data['config_info'];
		$db_data['quiz_type']=$this->lang->line('list');
		$data['content'] = $this->load->view($this->template.'/home/home',$db_data,true);
		$data['page_title'] = $db_data['config_info']->site_title;
		$data['page_description'] = $db_data['config_info']->page_description;
		$data['meta_url'] = $db_data['config_info']->facebook_url;
...
function articles($page=0)
	{	
		...
		$db_data['game_type']='articles';
		$db_data['ref_page']="articles";
		...
		$db_data['random_games']=$this->home_model->get_randon_lists('games','4','','');
		$data['config_info'] = $db_data['config_info'];
		$db_data['quiz_type']=$this->lang->line('article');
		$data['content'] = $this->load->view($this->template.'/home/home',$db_data,true);
		$data['page_title'] = $db_data['config_info']->site_title;
		$data['page_description'] = $db_data['config_info']->page_description;
		$data['meta_url'] = $db_data['config_info']->facebook_url;
	...
	{	
		...
		$db_data['game_type']='videos';
		$db_data['ref_page']="videos";
		...
		$db_data['random_games']=$this->home_model->get_randon_lists('games','4','','');
		$data['config_info'] = $db_data['config_info'];
		$db_data['quiz_type']=$this->lang->line('video');
		$data['content'] = $this->load->view($this->template.'/home/home',$db_data,true);
		$data['page_title'] = $db_data['config_info']->site_title;
		$data['page_description'] = $db_data['config_info']->page_description;
		$data['meta_url'] = $db_data['config_info']->facebook_url;
...

	{
...

		{
			...
			$source_url =site_url().$game->alias.".html";
	...
	{	
		...
		/*else
		{
			...
			$tests_info=$this->home_model->get_game($tests_alias);
			//print_r($tests_info); exit;
			if(!$tests_info) redirect(site_url().'not-found.html'); 
			//$this->config->set_item('is_language_disable', 'yes');
		}*/
		...
		$db_data['questions']=$this->home_model->get_questions($db_data['tests_info']->tests_id);

		if(!empty($_GET['ref_page']) && $_GET['ref_page'] == 'top') $db_data['ref_page']="top";
		
		//$test_view_image=$this->home_model->get_test_image($tests_info->testid);
		//$db_data['test_view_image']=$test_view_image;
		$banner_ids=$this->home_model->get_test_banner_ids($tests_info->tests_id);
		if($tests_info->is_real_test == 2)
		{
			/*if($banner_ids)
			{
				$db_data['banner_list']=$this->home_model->get_test_banners($banner_ids);
			}
			else
			{
				$db_data['banner_list']=$this->home_model->get_banners('twist');
			}
			
			$db_data['top_banner']=$this->home_model->get_topbanners('4');*/
			$db_data['sidebar_widgets']=$this->home_model->get_widget('twistsidebar',$this->session->userdata('lang_code'),$tests_info->tests_id);
			$db_data['bottom_widgets']=$this->home_model->get_widget('twistbottom',$this->session->userdata('lang_code'),$tests_info->tests_id);
		...
		else if($tests_info->is_real_test == 3)
		{
			...
			{
				$db_data['banner_list']=$this->home_model->get_banners('3apps');
			}
			
			$db_data['sidebar_widgets']=$this->home_model->get_widget('namesidebar',$this->session->userdata('lang_code'),$tests_info->tests_id);
			$db_data['bottom_widgets']=$this->home_model->get_widget('namebottom',$this->session->userdata('lang_code'),$tests_info->tests_id);
		...
		else if($tests_info->is_real_test == 6)
		{
			...
			{
				$db_data['banner_list']=$this->home_model->get_banners('facebook_apps',21);
			}*/
			//$db_data['random_fb_apps']=$this->home_model->get_randon_lists('games','4',$tests_info->tests_id,'6');
			
			$fb_soulmate_result=false;
			if(isset($_GET['result']))
			{
				$fb_soulmate_result=$this->home_model->get_fb_soulmate_result(trim($_GET['result']));
				$db_data['test_bg_image']=base_url().$fb_soulmate_result->image_name;
				$db_data['friend_name']=$fb_soulmate_result->friend_name;
				$db_data['id']=trim($_GET['result']);
				
			}
			
			$db_data['sidebar_widgets']=$this->home_model->get_widget('fasidebar',$this->session->userdata('lang_code'),$tests_info->tests_id);
			$db_data['bottom_widgets']=$this->home_model->get_widget('fabottom',$this->session->userdata('lang_code'),$tests_info->tests_id);
			$db_data['ads_info']=$this->home_model->get_ads_unique('facebook_apps');
			if($fb_soulmate_result)
			{
				$data['content'] = $this->load->view($this->template.'/home/facebook_apps_test_result_view',$db_data,true);
			}
			else
			{
		...
		{
			$db_data['sidebar_widgets']=$this->home_model->get_widget('personalsidebar',$this->session->userdata('lang_code'),$tests_info->tests_id);
			$db_data['bottom_widgets']=$this->home_model->get_widget('personalbottom',$this->session->userdata('lang_code'),$tests_info->tests_id);
			$db_data['ads_info']=$this->home_model->get_ads_unique('personal');
		...
		//----- this is for soulmate fb app result share
		$fb_r_id="";
		if(isset($_GET['fb_r'])) $fb_r_id=$_GET['fb_r'];
		if($fb_r_id)
		{ 
			$fb_soulmate_result=$this->home_model->get_fb_soulmate_result($fb_r_id);
			$data['meta_title'] =$db_data['tests_info']->title;
			$data['meta_img'] =base_url().$fb_soulmate_result->image_name;
			$data['fb_meta_description'] =$db_data['tests_info']->fbshare_des;
			$data['meta_url'] = base_url().$db_data['tests_info']->alias.".html?fb_r=".$fb_r_id;
			$data['meta_img_width']="600";
			$data['meta_img_height']="315";
		}
	...
	{	
		...
		if($result_id)
		{
			...
			$data['meta_img'] = base_url()."home/create_image/".$name."/".$res_id."/".$tests_info->alias;
			$data['meta_title'] =strtolower(str_replace("?","!",$db_data['tests_info']->title));
			$data['fb_meta_description'] = $db_data['tests_info']->title;
			$data['meta_url'] = site_url().$db_data['tests_info']->alias."/".$result_id.".html";
			$data['page_title'] =strtolower(str_replace("?","!",$db_data['tests_info']->title));
			//$data['page_title'] ="Testing.......";
			if(!empty($_GET['ref_page']) && $_GET['ref_page'] == 'top') $data['active_menu']='top';
			else $data['active_menu']='';
			$data['cur_page'] ="view";
	...
	{	
...

		{
			...
			$db_data['result_options']=$this->home_model->get_result_options($db_data['tests_info']->testid,$db_data['tests_info']->lang_code);		

			

			if(!empty($_GET['ref_page']) && $_GET['ref_page'] == 'top') $db_data['ref_page']="top";
			
			$db_data['banner_list']=$this->home_model->get_banners('personal',6);
...

			{
				...
				$db_data['questions']=$this->home_model->get_questions($tests_info->tests_id);
	...
	function list_view($tests_alias,$active_menu='')
	{
		...
		$db_data=array();
		$db_data['config_info']=$this->home_model->get_config('config',$this->session->userdata('lang_code'));
		$db_data['ads_info']=$this->home_model->get_ads_unique('list');
	...
	function article($tests_alias,$active_menu='')
	{
		...
		$db_data=array();
		$db_data['config_info']=$this->home_model->get_config('config',$this->session->userdata('lang_code'));
		$db_data['ads_info']=$this->home_model->get_ads_unique('ariticle');
	...
	function video($tests_alias,$active_menu='')
	{
		...
		$db_data=array();
		$db_data['config_info']=$this->home_model->get_config('config',$this->session->userdata('lang_code'));
		$db_data['ads_info']=$this->home_model->get_ads_unique('video');
		$tests_info=$this->home_model->get_game($tests_alias);
		if(!$tests_info) redirect(site_url().'not-found.html'); 
		$db_data['tests_info']=$tests_info;
	...
	function ajax_load()
	{
		...
		$game_type='all';
		if($this->input->post('game_type')) $game_type=$this->input->post('game_type');
		
		if($game_type == 'quiz')
		{
			$game_list=$this->home_model->get_games($start,$limit,'','',$game_type);
		}
		
		else if($game_type == 'trivia')
		{
			$game_list=$this->home_model->get_trivis($start,$limit);
		}
		else if($game_type == 'apps')
		{
			$game_list=$this->home_model->get_apps($start,$limit);
		}
		else
	...
	function create_image($name='',$result_id=0,$test_alias)
	{
		...
		$result_info=$this->home_model->get_result_option($result_id,$this->session->userdata('lang_code'));
		
		$test_info=$this->home_model->get_game_by_testid($result_info->test_id,$this->session->userdata('lang_code'));
		$test_title=$test_info->title;
		
		$en_result_info=$this->home_model->get_default_result_option($result_info->result_optionid,'en');
		//echo "<pre>";
		//print_r($en_result_info); exit;
		
		//print_r($test_info); exit;
		$im =  imagecreatefrompng("assets/img/test_result_img/".$en_result_info->test_result_img);
		list($o_width, $o_height) = getimagesize("assets/img/test_result_img/".$en_result_info->test_result_img);
		
		$font = 'assets/fonts/Arial Regular.ttf';
		$font_size = 20;
		//--------test title...........
		$test_title_bg = imagecolorallocate($im, 143, 44, 186);
		$test_title_color=imagecolorallocate($im, 255, 255, 255);
		
		$text_box=$this->calculateTextBox($test_title,$font,$font_size,0); 
		$text_width  = $text_box['width'];
		$text_height  = $text_box['height'];
		//echo $text_height; exit;
		$test_title_box_height=$text_height+12;
		imagefilledrectangle($im, 0, 0, $o_width, $test_title_box_height+5, $test_title_bg);
		$x = floor(($o_width  - $text_width)/2); 
		$y = ($test_title_box_height) / 2 + $text_height/2;
		
		imagettftext($im, $font_size, 0, $x-4, $y, $test_title_color, $font, $test_title);
		
		$test_title_y=$test_title_box_height + 10;
		//echo $test_title_y; exit;
		//echo "I am here"; exit;
		...
		//imagefilledrectangle($im, 4, $test_title_y, 735, $title_box_height+5+$test_title_y, $black);
		//imagefilledrectangle($im, 4, $test_title_y, 735, $title_box_height+$test_title_y, $white);
		
		// end title
	   
	 	$x = floor(($o_width  - $text_width)/2); 
		$y = ($title_box_height) / 2 + $text_height/2 + $test_title_y;
		//$y-=6;
		...
		$titel_y=$title_box_height+32+$test_title_y; 
		...
		$text_width  = $text_box['width'];
		$text_height  = $text_box['height'];
		$box_width=$text_width + 27;
		$x= floor(($o_width  - $box_width)/2); 
		
		$y = $titel_y + $text_height/2 +7.5;
		$color = imagecolorallocate ($im, 255, 255, 255);
		$black = imagecolorallocate($im, 000, 000, 000);
		
		$result_box_y1=$title_box_height + $test_title_y + 15;
	...
	function photoreconize_how_old()
	{
		...
		$test_info=$this->home_model->get_game_by_id($test_id);
		...
		foreach($response->face as $face)
		{
			...
			echo "<br>";*/
			//echo $point;
			$result_options=$this->home_model->get_result_options($test_info->tests_id,$test_info->lang_code);	
		...
		if ($im !== false) {
			...
			$db_data['banner_list']=$this->home_model->get_banners('how_old');
	...
	function photoreconize_face()
	{
		...
		if ($im !== false) {
			...
			$db_data['test_info']=$this->home_model->get_game($alias);
			$db_data['result_set']=$result_set;
			$db_data['banner_list']=$this->home_model->get_banners('face_test');
	...
	function facebook_apps()
	{
		...
		//echo $bg_img;
		if($bg_img_type  == 'png') $im=imagecreatefrompng($bg_img);
		if($bg_img_type  == 'jpg' || $bg_img_type  == 'jpeg') $im=imagecreatefromjpeg($bg_img);
		if($bg_img_type  == 'gif') $im=imagecreatefromgif($bg_img);
		...
		if($bg_img_type  == 'png') imagepng($im,$file_name);
		if($bg_img_type  == 'jpg' || $bg_img_type  == 'jpeg') imagejpeg($im,$file_name);
		if($bg_img_type  == 'gif') imagegif($im,$file_name);
