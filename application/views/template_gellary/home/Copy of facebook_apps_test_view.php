...
<div style="background-color:#FFFFFF; margin-bottom:20px; padding-bottom:20px;">
	...
	<div id="main_container22" class="col-xs-12 col-sm-8 col-md-8 left_part ">
		...
		<div id="fb_apps_container">
			<div id="fb_apps_info">		
				<div class="">
					<img class="fb_bg_image" src="<?php echo base_url(); ?>assets/img/image/<?php echo $tests_info->image; ?>" />
				</div>
				
				<div class="fb_test_description"><?php echo $tests_info->description;?></div>
				<div id="start" class="qc-fb-share"><img src="<?php echo base_url();?>assets/img/fb-logo.png"><?php echo $this->lang->line('find_true_friend_btn');?></div>
			</div>
		...
		<?php /*?><h3 class="may_like">You May Like</h3>
		<div class="row">
		<?php
			$items=$this->home_model->get_randon_lists('all',10);
			if($items)
			{ 
				...
				{ 
					...
					?>
						<div class="col-xs-12 col-sm-6 col-md-6 latest_item_box">
							<a href="<?php echo $url;?>">
		...
		<?php if($banner_list) { ?><div class="rubric-title"><?php echo $this->lang->line('you_may_like');?></div> <?php } ?>
		<div class="row22">
		<?php
					...
					?>
						<div class="col-xs-12 col-sm-6 col-md-4 banner_item_box">
							<a href="<?php echo $banner->url;?>">
								<img class="img-responsive"src="<?php echo base_url();?><?php echo $banner->image;?>" />
								<div class="banner_title">
									<?php echo $banner->title;?>
								</div>
								<div class="banner_brand_title">
									<?php echo $banner->brand_title;?>
								</div>
	...
	<div class="col-xs-12 col-sm-4 col-md-4 right_part" style="min-height:500px;">
		<div class="well right_content" style="margin-top:25px;">
		<?php
			//echo "<pre>";
			//print_r($config_info); exit;
		?>
		<?php if($ads_info->test_sitebar_1_adsense){ ?><div class="sidebar_adsense"><?php echo htmlspecialchars_decode($ads_info->test_sitebar_1_adsense); ?></div> <?php } ?>
		...
		<?php if($ads_info->test_sitebar_2_adsense){ ?><div class="sidebar_adsense"><?php echo htmlspecialchars_decode($ads_info->test_sitebar_2_adsense); ?></div> <?php } ?>
		...
		<div class="more_title " style="padding:25px 0px;"><?php echo $this->lang->line('popular_qizzes');?></div>
			<?php
			$items=$this->home_model->get_topgames(5);
			if($items)
			{ 
				...
				{ 
					...
					?>
						<div>
							...
							<p class="latest_games_caption list_of_item_title" style="padding:15px;">
...
<script>
	...
	function fbShare(position) {
		if ($(window).width() < 700) {position = "mobile." + position;}
		if (!has_shared) {ga('send', 'event', 'share_' + position,'soulmate', winid);has_shared = true;}
		href = "https://www.facebook.com/sharer/sharer.php?u=http://www.captainquizz.com/special/soulmate/r/"+fbid+rand;
	...
	function CheckFbPost(user) {
		FB.api("/me/posts",{fields: ['comments', 'likes', 'with_tags', 'from', 'to'], limit:40},function(response) {
	...
	function CheckFBPhotos(user) {
		...
		FB.api("/me/photos",{limit:30},function(response) {
			if (response && response.data) {
	...
	function filter_Fb_friends(user) {
		...
		FB.api("/",{ids: ids, fields:'metadata{type}', metadata:1},function(response) { 
			$("#loading>span").html('Analyzing potential friends...');
