	...
	<table id="mytable" class="table table-striped table-hover table-fixed-header"> 
		<thead class="header">
			...
			<!--<th>Page Title</th>-->
			<?php if(($this->session->userdata('user_type') != 2) and ($this->session->userdata('user_type') != 4)) { ?><th>Category Name</th><?php } ?>
			<?php if($this->session->userdata('user_type') !=4 ) { ?> <th>Author</th><?php } ?>
			...
			<?php
				...
				{	
					...
					<tr>
						...
						<?php if(($this->session->userdata('user_type') != 2) and ($this->session->userdata('user_type') != 4)) { ?><td data-title="Category Name"><?php echo $row->category_name;?></td><?php } ?>
						<?php if($this->session->userdata('user_type') !=4 ) { ?><td data-title="Category Name"><a href="<?php echo site_url();?>admin/tests/index/?user_id=<?php echo $row->created_by;?>"><?php echo $row->first_name;?> <?php echo $row->last_name;?></a></td><?php } ?>
						...
						<td data-title="Action">
							<?php if($this->session->userdata('user_type') !=4 ) { ?><a href="<?php echo site_url();?>admin/tests/test_edit/<?php echo $row->tests_id;?>"><span title="Edit" class="glyphicon glyphicon-pencil"></span> </a> &nbsp; <?php } ?>
							<?php if($this->session->userdata('user_type') != 2 and $this->session->userdata('user_type') != 4) { ?><a href="<?php echo site_url();?><?php echo $row->alias;?>.html" target="_blank"><span title="View" class="glyphicon glyphicon-search"></span></a> &nbsp; <?php } ?>
							<a href="<?php echo site_url();?>admin/tests/test_details/<?php echo $row->tests_id;?>"><span class="glyphicon glyphicon-eye-open" title="Preview"></span></a> &nbsp; 
							<?php if($this->session->userdata('user_type') !=4 ) { ?><a href="<?php echo site_url();?>admin/tests/result_option/<?php echo $row->tests_id;?>"><span class="glyphicon glyphicon-wrench" title="Result"></span></a> &nbsp; <?php } ?>
							<?php if($this->session->userdata('user_type') !=4 ) { ?><a href="<?php echo site_url();?>admin/tests/questions/<?php echo $row->tests_id;?>"><span class="glyphicon glyphicon-question-sign" title="Quesitons"></span></a> &nbsp; <?php } ?>
							 
							<?php if($this->session->userdata('user_type') != 2 and $this->session->userdata('user_type') !=4) { ?><span class="glyphicon glyphicon-camera" style="color:<?php echo $camera_color;?>;"></span> &nbsp;  <?php } ?>
							<?php if($this->session->userdata('user_type') == 1) { ?>
